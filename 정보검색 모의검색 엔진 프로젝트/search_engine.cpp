...
int main() {
	...
	fin.seekg(static_cast<size_t>(26*wordFile["telescop"][3]));
	...
	cout << docID << endl;
	cout << TF << endl;
	cout<<fixed<<showpoint<<setprecision(4)<<weight << endl;
...
int readyForSearch() { //단어 정보 파일과 문서정보 파일을 메모리로 올리는 과정
	...
	while (fgets(line, 1000, docu)) {
		...
		token = strtok_s(line, "	", &context); i = atoi(token);
